package com.example.ioshacketonapp.shared.cache.shared

import com.example.ioshacketonapp.shared.cache.AppDatabase
import com.example.ioshacketonapp.shared.cache.AppDatabaseQueries
import com.example.ioshacketonapp.shared.cache.Rocket
import com.example.ioshacketonapp.shared.cache.SelectAllLaunchesInfo
...
private class AppDatabaseQueriesImpl(
  ...
  override fun selectRocketById(id: String): Query<Rocket> = selectRocketById(id) { id_, name,
      type ->
    Rocket(
      id_,
      name,
  ...
  override fun <T : Any> selectAllLaunchesInfo(mapper: (
    ...
    name: String?,
    type: String?
  ) -> T): Query<T> = Query(1648876787, selectAllLaunchesInfo, driver, "AppDatabase.sq",
  ...
  override fun insertLaunch(
    ...
    notifyQueries(-1824395379, {database.appDatabaseQueries.selectAllLaunchesInfo})
  }
  ...
  override fun insertRocket(
    ...
    notifyQueries(-1640230226, {database.appDatabaseQueries.selectRocketById +
        database.appDatabaseQueries.selectAllLaunchesInfo})
  }

  override fun removeAllLaunches() {
    driver.execute(-1009052131, """DELETE FROM Launch""", 0)
    notifyQueries(-1009052131, {database.appDatabaseQueries.selectAllLaunchesInfo})
  }

  override fun removeAllRockets() {
    driver.execute(-280965597, """DELETE FROM Rocket""", 0)
    notifyQueries(-280965597, {database.appDatabaseQueries.selectRocketById +
        database.appDatabaseQueries.selectAllLaunchesInfo})
  }
  ...
  private inner class SelectRocketByIdQuery<out T : Any>(
    ...
    override fun execute(): SqlCursor = driver.executeQuery(773428771, """
    |SELECT * FROM Rocket
    |WHERE id = ?
    """.trimMargin(), 1) {
      bindString(1, id)
